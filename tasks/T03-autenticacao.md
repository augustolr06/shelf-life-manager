...
## Critério de aceite
- [ ] `POST /auth/login` — valida email/senha (bcrypt), retorna JWT em cookie `httpOnly` + `secure` (em produção)
- [ ] `POST /auth/logout` — limpa o cookie
- [ ] Middleware Fastify que decodifica o JWT e injeta `usuario` no request
- [ ] Middleware de autorização por papel (`ATENDENTE`, `GESTOR`) reutilizável em qualquer rota
- [ ] Rota protegida de teste (`GET /auth/me`) retornando o usuário autenticado
- [ ] Testes cobrindo: login válido, senha incorreta, acesso a rota restrita sem papel adequado
